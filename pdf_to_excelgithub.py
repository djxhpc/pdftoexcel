...
import numpy as np  # 💡 引入 numpy 解決 RapidOCR 報錯
...
def ocr_cropped_image(pixmap_img, scale=2):
    ...
    # 💡 將 PIL Image 轉換為 RapidOCR 支援的 numpy array 格式
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for i, page in enumerate(pdf.pages):
            ...
            if page_num == 14:
                ...
                row_threshold = 14 * 3  # Y 軸像素判定閾值 (隨 3 倍放大調整)
                ...
                continue  # 第 14 頁處理完畢，跳過後面的常規處理流程
                    ...
                    for row in table:
                        for col_idx, cell in enumerate(row):
                            if cell is None or str(cell).strip() == "":
                                row[col_idx] = ""
                        ws.append(row)
                    current_row += len(table) + 1 # 隔開多個表格
                    
            else:
                # 2. 如果沒有結構化表格，嘗試用文字行（行對齊）的方式還原
                text_objects = page.extract_words()
                
                if not text_objects:
                    # 3. 完全沒有電子文字（純圖片 PDF）-> 啟動全頁 OCR
...
if __name__ == "__main__":
    input_pdf = r"D:\work\0601Yilan\1150213.pdf"       # 💡 請確認您的 PDF 檔名
